...
void Get_Gaussian_Kernel(float* kernel, int dim, double sigma) {
    ...
    #pragma omp parallel for collapse(2) private(i,j, value, idx) shared(kernel, dim, sigma) 
	for (i = -dim; i <= dim; i++) {
		for (j = -dim; j <= dim; j++) {
...
void Apply_Gaussian_Blur_Filter(float* kernel, int dim, Matrix *mtx) {
	...
	for (i = 0; i < mtx->height; i++) {
        ...
        #pragma omp parallel for private(j, ii, jj) schedule(static) reduction(+:RGBGY[:4])
